...
export type AdminAiConnectionTestResponse = { chat: AdminAiConnectionResult; embedding: AdminAiConnectionResult };
...
export type AdminMailSettings = { host: string; port: number; username?: string; passwordConfigured: boolean; smtpAuth: boolean; starttls: boolean; fromAddress: string; enabled: boolean; source: 'DATABASE' | 'ENVIRONMENT'; encryptionConfigured: boolean; updatedAt?: string };
export type AdminMailSettingsInput = { host: string; port: number; username?: string; password?: string; smtpAuth: boolean; starttls: boolean; fromAddress: string; enabled?: boolean };
...
export const adminApi = {
  ...
  mailSettings: () => request<AdminMailSettings>('/admin/mail-settings', {}, true),
  updateMailSettings: (body: AdminMailSettingsInput & { enabled: boolean }) => request<AdminMailSettings>('/admin/mail-settings', { method: 'PUT', body: JSON.stringify(body) }, true),
  testMailSettings: (body: AdminMailSettingsInput) => request<{ success: boolean; message: string }>('/admin/mail-settings/test', { method: 'POST', body: JSON.stringify(body) }, true),
