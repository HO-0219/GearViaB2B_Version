...
export function LandingPage() {
  ...
  return <main className="landing-page">
    <header className="landing-nav">
      <Link to="/" className="landing-brand"><BrandMark /><strong>{t('B2BGearVia', 'B2BGearVia')}</strong></Link>
    ...
    <section className="landing-hero">
      ...
      <div className="landing-hero-actions">
        <Link className="landing-primary" to="/login">{t('시작하기', 'Get started')} <span>→</span></Link>
        <Link className="landing-demo" to="/demo">{t('실제 화면 보기', 'View the product')} <span>↗</span></Link>
      </div>
      <button className="landing-pwa-hero" type="button" onClick={() => setInstallOpen(true)} disabled={installed}><span aria-hidden="true">↓</span>{installed ? t('B2BGearVia 앱으로 실행 중', 'Running as the B2BGearVia app') : t('앱스토어 없이, 이 기기에 B2BGearVia 추가', 'Add B2BGearVia to this device—no app store')}</button>
    ...
    <section className="landing-product" aria-label={t('제품 화면 미리보기', 'Product preview')}>
      <div className="landing-window-bar"><i /><i /><i /><span>B2BGearVia · PWA</span></div>
      <div className="landing-product-body">
        <aside><div className="mock-brand"><BrandMark />{t('B2BGearVia', 'B2BGearVia')}</div><span className="active">⌂ {t('홈', 'Home')}</span><span>♧ {t('그룹', 'Groups')}</span><span>□ {t('캘린더', 'Calendar')}</span><span>♢ {t('알림', 'Alerts')}</span><b>{t('로컬 알파 시연팀', 'Alpha demo team')}</b></aside>
    ...
    <section className="landing-ai-showcase" aria-label={t('AI 비서와 AI 리포트 미리보기', 'AI assistant and report preview')}>
      ...
      <div className="landing-ai-products">
        ...
        <article className="landing-report-preview"><div className="landing-ai-card-head"><span>↗</span><div><small>AI WEEKLY REPORT</small><strong>{t('숫자 너머의 변화와 위험까지', 'Changes and risks beyond the numbers')}</strong></div><b>R3</b></div><div className="landing-report-summary"><span>{t('7월 4주차 · 그룹 전체', 'July week 4 · Whole group')}</span><h3>{t('출시 준비는 계획대로 진행 중이지만, 배포 검증 업무가 일정 위험을 만들고 있습니다.', 'Launch preparation is on track, but Deployment verification is creating schedule risk.')}</h3><div><p><small>{t('완료율', 'Completion')}</small><strong>76%</strong><i>+5%p</i></p><p><small>{t('완료 업무', 'Completed')}</small><strong>19</strong><i>+6</i></p><p><small>{t('위험 업무', 'At risk')}</small><strong>3</strong><i className="risk">{t('확인', 'Review')}</i></p></div><ul><li><b>{t('위험', 'RISK')}</b><span>{t('배포 환경 변수 검증이 오늘 마감이며 체크리스트 1개가 남았습니다.', 'Deployment environment review is due today with one checklist item remaining.')}</span></li><li><b>{t('제안', 'ACTION')}</b><span>{t('오후 배포 전 정품질 담당자의 검증 결과를 먼저 확인하세요.', 'Review the QA owner’s verification result before this afternoon’s release.')}</span></li></ul></div></article>
      </div>
      <footer><span>{t('AI 비서와 AI 리포트는 관리자가 활성화한 팀에서 사용할 수 있습니다.', 'AI assistant and reports are available in teams enabled by an admin.')}</span><div><Link to="/demo">{t('AI 데모 체험', 'Try the AI demo')} →</Link><Link to="/pricing">{t('AI 설정 보기', 'View AI setup')}</Link></div></footer>
    </section>
    ...
    <section className="landing-name-story" aria-label={t('B2BGearVia 브랜드 이야기', 'The B2BGearVia name')}>
      <span>Gear + Via</span>
    ...
    <section className="landing-proof" aria-label={t('B2BGearVia 핵심 가치', 'B2BGearVia key value')}>
