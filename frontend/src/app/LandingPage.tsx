...
export function LandingPage() {
  ...
  return <main className="landing-page">
    ...
    <section className="landing-final-cta"><BrandMark /><h2>{t('팀의 업무를 하나의 흐름으로 연결하세요.', 'Connect your team in one clear workflow.')}</h2><p>{t('요청부터 실행과 보고까지, B2BGearVia에서 함께 움직입니다.', 'Move together from request to execution and reporting with B2BGearVia.')}</p><div><Link to="/login">{t('무료로 시작', 'Get started')} →</Link><Link to="/demo">{t('데모 보기', 'View demo')}</Link></div></section>
    <footer><Link to="/" className="landing-brand"><BrandMark /><strong>{t('B2BGearVia', 'B2BGearVia')}</strong></Link><nav><Link to="/privacy">{t('개인정보 처리방침', 'Privacy')}</Link><Link to="/terms">{t('이용약관', 'Terms')}</Link><Link to="/paid-terms">{t('유료서비스 약관', 'Paid terms')}</Link><Link to="/refund-policy">{t('환불 정책', 'Refunds')}</Link><Link to="/site-map">{t('사이트맵', 'Site map')}</Link></nav><small>© 2026 B2BGearVia</small></footer>
