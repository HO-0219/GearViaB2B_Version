...
const tasks: DemoTask[] = [
  ...
  { id: 2, ko: '배포 환경 변수 검증', en: 'Deployment environment review', status: 'IN_PROGRESS', priority: 'URGENT', owner: '정품질', due: ['오늘 17:00', 'Today 17:00'], description: ['환경 변수와 비밀값이 응답 및 로그에 노출되지 않는지 확인합니다.', 'Confirm environment variables and secrets never appear in responses or logs.'] },
  ...
  { id: 12, ko: '고객 문의 FAQ 작성', en: 'Write customer FAQ', status: 'IN_PROGRESS', priority: 'NORMAL', owner: '박지원', due: ['어제', 'Yesterday'], description: ['로그인, 초대와 설치 관련 질문을 정리합니다.', 'Cover common login, invitation, and installation questions.'] },
];
...
export function DemoPage() {
  ...
  return <div className="demo-page">
    <aside className="demo-nav">
      <Link className="demo-brand" to="/"><span><BrandMark /></span><strong>{t('B2BGearVia', 'B2BGearVia')}</strong></Link>
      ...
      <div className="demo-profile"><span>김</span><div><strong>{t('김서준 팀장', 'Seo-jun Kim')}</strong><small>demo@b2bgearvia.local</small></div></div>
    </aside>
...
function DemoHome({ onOpenGroup, onOpenTasks, onOpenAlerts }: { onOpenGroup: () => void; onOpenTasks: () => void; onOpenAlerts: () => void }) {
  ...
  return <section className="demo-view">
    ...
    <div className="demo-home-grid">
      <article className="demo-panel demo-home-group"><header><div><span>SHORTCUTS</span><h2>{t('바로가기', 'Shortcuts')}</h2></div></header><button type="button" onClick={onOpenGroup}><span>G</span><div><strong>{t('B2BGearVia 런칭 준비팀', 'B2BGearVia Launch Team')}</strong><small>{t('팀 대시보드 열기', 'Open group dashboard')}</small></div><b>›</b></button></article>
      ...
      <article className="demo-panel demo-upcoming"><header><div><span>SCHEDULE</span><h2>{t('다가오는 일정', 'Upcoming events')}</h2></div></header><div><span><b>31</b><small>JUL</small></span><p><strong>{t('주간 진행 공유', 'Weekly progress sync')}</strong><small>10:00 · {t('B2BGearVia 런칭 준비팀', 'B2BGearVia Launch Team')}</small></p></div><div><span><b>01</b><small>AUG</small></span><p><strong>{t('출시 전 보안 점검', 'Pre-launch security review')}</strong><small>14:00 · Online</small></p></div></article>
      <article className="demo-panel demo-home-alerts"><header><div><span>ALERTS</span><h2>{t('미확인 알림', 'Unread alerts')}</h2></div><button type="button" onClick={onOpenAlerts}>{t('전체 보기', 'View all')} →</button></header><button type="button" onClick={onOpenAlerts}><strong>{t('업무 마감 임박', 'Task due soon')}</strong><small>{t('배포 환경 변수 검증이 오늘 마감됩니다.', 'Deployment environment review is due today.')}</small></button><button type="button" onClick={onOpenAlerts}><strong>{t('새 업무 승인 요청', 'New approval request')}</strong><small>{t('이메일 템플릿 개선 제안이 승인을 기다립니다.', 'The email template proposal is waiting for approval.')}</small></button></article>
    </div>
  </section>;
}
...
function DemoMembers({ onBack, onOpenTasks }: { onBack: () => void; onOpenTasks: () => void }) {
  const { t } = useLanguage();
  return <section className="demo-view">
    <header className="demo-view-heading"><div><button className="demo-back-link" type="button" onClick={onBack}>← {t('그룹 대시보드', 'Group dashboard')}</button><span className="page-eyebrow">TEAM MEMBERS</span><h1>{t('팀원', 'Team members')}</h1><p>{t('B2BGearVia 런칭 준비팀에서 함께하는 사람과 역할을 확인하세요.', 'See everyone working in the B2BGearVia Launch Team and their roles.')}</p></div><div className="demo-heading-actions"><button type="button" onClick={onOpenTasks}>{t('업무 보기', 'View tasks')}</button><button type="button" disabled>{t('초대·설정', 'Invites & settings')}</button></div></header>
...
function DemoDashboard({ onOpenTasks, onOpenMembers, onSelect }: { onOpenTasks: () => void; onOpenMembers: () => void; onSelect: (task: DemoTask) => void }) {
  ...
  return <section className="demo-view">
    ...
    <article className="demo-panel demo-report-panel"><header><div><span>AI REPORTS</span><h2>{t('AI 업무 리포트', 'AI work report')}</h2><p>{t('저장된 업무 기록에서 핵심 변화, 위험과 다음 행동을 찾아 주간 리포트로 정리합니다.', 'Turn recorded work into a weekly brief of key changes, risks, and next actions.')}</p></div><b>{t('조직 정책 활성', 'Organization policy enabled')}</b></header><div><label><span>{t('범위', 'Scope')}</span><select defaultValue="GROUP"><option value="GROUP">{t('그룹 전체', 'Whole group')}</option></select></label><label><span>{t('기간', 'Period')}</span><select defaultValue="WEEKLY"><option value="WEEKLY">{t('7월 4주차', 'July week 4')}</option></select></label><button className="demo-ai-report-button" type="button" onClick={() => setReportOpen((value) => !value)}>{reportOpen ? t('미리보기 닫기', 'Close preview') : t('AI 리포트 보기', 'View AI report')}</button></div>{reportOpen && <div className="demo-ai-report"><header><div><span>{t('주간 실행 브리프', 'WEEKLY EXECUTIVE BRIEF')}</span><h3>{t('출시 준비는 계획대로 진행 중이지만, 배포 검증 업무가 일정 위험을 만들고 있습니다.', 'Launch preparation is on track, but Deployment verification is creating schedule risk.')}</h3></div><b>R3</b></header><div className="demo-ai-report-kpis"><p><small>{t('완료율', 'Completion')}</small><strong>76%</strong><i>+5%p</i></p><p><small>{t('완료 업무', 'Completed')}</small><strong>19</strong><i>+6</i></p><p><small>{t('지연·위험', 'Delayed / at risk')}</small><strong>3</strong><i className="risk">{t('확인 필요', 'Review')}</i></p></div><div className="demo-ai-report-grid"><section><b>{t('핵심 위험', 'KEY RISK')}</b><h4>{t('배포 검증 결과가 배포 결정의 선행 조건입니다.', 'Deployment verification is blocking the release decision.')}</h4><p>{t('마스킹 검증이 오늘 마감이며 체크리스트 1개가 남았습니다. 담당자 진행률은 75%입니다.', 'Masking verification is due today with one checklist item remaining. Owner progress is 75%.')}</p><small>#2 {t('배포 환경 변수 검증', 'Deployment environment review')}</small></section><section><b>{t('다음 행동', 'NEXT ACTION')}</b><h4>{t('오후 배포 전 검증 결과를 먼저 확인하세요.', 'Review verification before this afternoon’s release.')}</h4><p>{t('정품질 담당자의 최종 결과 공유 후 운영 배포 체크리스트를 승인하는 순서가 안전합니다.', 'Wait for the QA owner’s final result before approving the production checklist.')}</p><small>{t('담당 김서준 · 오늘 15:00', 'Owner Seo-jun Kim · Today 15:00')}</small></section></div></div>}<small>ⓘ {t('데모에서는 샘플 분석만 보여주며 실제 AI 생성·다운로드·메일 발송은 실행하지 않습니다.', 'The demo shows a sample analysis; AI generation, downloads, and email delivery do not run.')}</small></article>
  </section>;
}
...
function DemoCalendar() {
  ...
  const eventForDay = (day: Date) => {
    ...
    const values = [
      ...
      [23, '15:30', t('배포 보안 검토', 'Deployment security review'), 'task'],
...
function DemoNotifications({ onOpenTask }: { onOpenTask: (task: DemoTask) => void }) {
  const { t } = useLanguage();
  const items = [
    ['urgent', t('업무 마감 임박', 'Task due soon'), t('배포 환경 변수 검증 업무가 오늘 마감됩니다.', 'Deployment environment review is due today.'), '12분 전', tasks[1]],
