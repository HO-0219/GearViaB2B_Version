...
export function GroupDashboardPage() {
  ...
  return <><AppNavigation /><main className="group-dashboard-page app-page">
    <header className="dashboard-header"><div><Link to="/groups">← {t('내 그룹', 'My groups')}</Link><h1>{dashboard?.groupName ?? t('그룹', 'Group')} {t('대시보드', 'Dashboard')}</h1><p>{t(`${periodLabel}의 업무와 일정을 모아봤어요.`, `Tasks and events for ${periodLabel}.`)}</p></div><div className="group-dashboard-actions"><button className="primary create-action" type="button" onClick={() => setShowCreate(true)}><span aria-hidden="true">＋</span> {t('새 업무', 'New task')}</button><Link className="danger button-link dashboard-emergency-link" to={`/groups/${groupId}/emergency-issues`}>! {t('긴급 이슈', 'Urgent issue')}</Link><Link className="secondary dashboard-project-link" to={`/groups/${groupId}/projects`}>{t('프로젝트', 'Projects')}</Link><Link className="secondary dashboard-files-link" to={`/groups/${groupId}/files`}>{t('파일', 'Files')}</Link><Link className="secondary dashboard-tasks-link" to={`/groups/${groupId}/tasks`}>{t('업무 목록', 'Tasks')}</Link><Link className="settings-icon-button" to={`/groups/${groupId}`} aria-label={t('그룹 설정', 'Group settings')}>⚙</Link></div></header>
