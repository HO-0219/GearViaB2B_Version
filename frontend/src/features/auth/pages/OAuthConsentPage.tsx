...
export function OAuthConsentPage() {
  ...
  return <AuthLayout title={t('Google 가입 동의', 'Complete Google sign-up')} description={t(
    'Google에서 확인한 정보를 검토하고 B2BGearVia 가입 동의를 선택해 주세요.',
    'Review the Google account information and choose your B2BGearVia consents.',
  )}>
    {profile ? <form className="form" onSubmit={submit}>
      ...
      <p className="oauth-data-note">{t(
        'B2BGearVia는 Google 비밀번호와 Google 액세스·리프레시 토큰을 저장하지 않습니다. 인증에 필요한 Google 계정 식별자, 이메일, 이름만 전달받습니다.',
        'B2BGearVia does not store your Google password or Google access/refresh tokens. It receives only the Google account identifier, email, and name needed for authentication.',
      )}</p>
      <fieldset className="signup-consents"><legend>{t('약관 및 수신 설정', 'Terms and preferences')}</legend>
        ...
        <Consent checked={consents.termsAgreed} onChange={value => setConsent('termsAgreed', value)} label={t('[필수] 서비스 이용약관 동의', '[Required] Terms of service')} href="/terms">
          {t('B2BGearVia의 업무·그룹·알림 기능 이용 조건, 계정 관리, 금지 행위 및 서비스 책임 범위를 확인하고 동의합니다.', 'I accept the conditions for B2BGearVia task, group, and notification features, account management, prohibited conduct, and service responsibilities.')}
        </Consent>
