...
export function AdminPage() {
  ...
  return <main className="admin-page"><header><div><span className="page-eyebrow">RESTRICTED OPERATIONS</span><h1>B2BGearVia Admin</h1><p>{t('별도 포트·서버 허용 IP·ADMIN 권한·MFA를 모두 통과한 운영 화면입니다.', 'This console requires the dedicated port, 서버 allowlist, ADMIN role, and MFA.')}</p></div><a href="/app">{t('서비스로 돌아가기', 'Back to app')}</a></header>
    ...
    <AdminTable title={t('그룹 현황', 'Group status')} emptyLabel={t('표시할 그룹이 없습니다.', 'No groups to display.')} headers={['ID', t('그룹', 'Group'), t('유형', 'Type'), t('활성 멤버', 'Active members'), t('플랜', 'Plan'), t('리포트 예약', 'Report schedule'), t('생성일', 'Created')]} rows={groups.map((group) => [group.id, group.name, group.type, group.activeMembers, group.membershipPlan, <StatusBadge value={group.reportScheduleActive ? t('사용', 'Active') : t('미사용', 'Off')} />, formatTime(group.createdAt)])} />
