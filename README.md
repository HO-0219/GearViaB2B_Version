...
- 체크리스트, 댓글, 멘션, 캘린더, 알림
- 프로젝트, 프로젝트 이슈, 긴급 이슈
- 그룹 채팅, 자료 공유, 문서 업로드, 그룹 단위 통합 파일 탐색기
- 대시보드, 기본 리포트, PDF 출력
- 로컬 계정, JWT 세션, 관리자 MFA
- 선택형 OpenAI 기반 AI 비서와 AI 리포트, 업로드 문서 기반 RAG 검색
- 로컬 디스크 또는 NAS/사내 스토리지 서버 연동(선택)
- 조직 로고·이름 커스터마이징, 전체 팀장 공지 발송
- 관리자 콘솔: 사용자·업무 관리, 로그인 이력, 감사 로그, 시스템 리소스 및 AI 사용량 모니터링
