...
if [[ ! "$db_url" =~ ^jdbc:mysql://(localhost|127\.0\.0\.1)(:([0-9]+))?/b2bgearvia([?].*)?$ ]]; then
  echo "안전상 localhost의 b2bgearvia 개발 DB에서만 시연 데이터를 만들 수 있습니다." >&2
  exit 1
fi
...
MYSQL_PWD="$db_password" mysql --protocol=TCP -h "$db_host" -P "$db_port" -u "$db_user" \
  --default-character-set=utf8mb4 b2bgearvia < "$seed_file"
